    ...
    return (
      <Modal title="Security" onClose={onClose}>
        <div className="flex items-center gap-3 bg-mint/10 border border-mint/30 rounded-2xl p-4 mb-4">
          ...
          <div className="text-[12.5px] text-[#C7CBDA]">PIN lock is currently <span className="text-mint font-semibold">enabled</span> on this device.</div>
        </div>
    ...
    <Modal title={stage === "create" ? "Set a PIN" : "Confirm PIN"} onClose={onClose}>
      <div className="flex flex-col items-center py-4">
        ...
        <div className="flex gap-3 mb-7">
          {[0, 1, 2, 3].map((i) => <div key={i} className={`w-3.5 h-3.5 rounded-full ${i < val.length ? "bg-violet" : "bg-[#262B3B]"}`} />)}
        </div>
